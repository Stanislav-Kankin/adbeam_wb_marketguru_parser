...
## Вкладка AdBeam: аудит сайтов из Excel

В GUI добавлена отдельная вкладка `Аудит сайтов`. Она переносит текущий AdBeam Excel Parser внутрь этого проекта без смешивания с WB-пайплайном.

Что делает вкладка:

- читает `.xlsx` и находит колонку с сайтом;
- показывает JSON-preview по Excel;
- последовательно обходит сайты через `httpx`;
- классифицирует сайты по rule-based ecom/direct сигналам;
- сохраняет новый Excel с колонками `AdBeam статус`, `AdBeam причина`, `AdBeam HTTP`, `AdBeam WB`, `AdBeam Ozon`, `AdBeam MP links found`;
- подсвечивает строки по итоговому статусу.

Для этой вкладки добавлены зависимости `httpx`, `selectolax` и `tldextract`.

## Сборка в исполняемый файл (.exe)

Сборка делается из текущего проекта через PyInstaller.
...
После сборки exe будет лежать в папке:

```text
dist\WB_INN_Extractor\WB_INN_Extractor.exe
```

Важно:
- при первой сборке скрипт сам ставит `pyinstaller`
- Chromium для Playwright тоже ставится автоматически командой `playwright install chromium`
- это оконная сборка GUI без консоли
